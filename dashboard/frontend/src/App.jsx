...
const NODE_NAMES = ['NODE_A','NODE_B','NODE_C','NODE_D','NODE_E','NODE_F',
...
export default function App() {
  ...
  const addLog = useCallback((msg, type = 'info') => {
    ...
    setLog(l => [...l.slice(-199), { ts, msg, type }])
  }, [])
  ...
  useEffect(() => {
    ...
    if (!wrap) return
    wrap.scrollLeft = (GRID_W - wrap.clientWidth) / 2
    wrap.scrollTop  = (GRID_H - wrap.clientHeight) / 2
  }, [])
  ...
  useEffect(() => {
    const connect = () => {
      ...
      ws.onmessage = (e) => {
        ...
        // Beacon registry — positions pushed from device on connect
        if (pkt.type === 'beacon_list') {
          setNodes(prev => {
            ...
            pkt.beacons.forEach(b => {
              next[b.name] = {
                ...(prev[b.name] || {}),
                name: b.name, mac: b.mac || '',
                major: b.major ?? 0, minor: b.minor ?? 0,
          ...
          addLog('Received ' + (pkt.beacons?.length ?? 0) + ' beacon positions from device', 'ok')
          return
        }
        ...
        setLiveData(prev => {
          const next = { ...prev }
          pkt.nodes.forEach(n => { next[n.name] = n })
          return next
        })
        // Only auto-place nodes not already in the registry
        setNodes(prev => {
          const next = { ...prev }
          pkt.nodes.forEach(n => {
            if (!next[n.name] && n.rssi !== 0) {
              next[n.name] = {
                name: n.name, mac: '', major: 0, minor: 0,
                x: +(Math.random() * 6 - 3).toFixed(1),
                y: +(Math.random() * 6 - 3).toFixed(1),
                z: 0, left: '', right: '', placed: false,
              }
            }
          })
          return next
  ...
  useEffect(() => {
    ...
    setForm({
      name: n.name, mac: n.mac || '', major: n.major ?? 0, minor: n.minor ?? 0,
  ...
  const removeNode = async () => {
    ...
    try {
      ...
      addLog(`Remove request failed: ${err.message}`, 'err')
    }
  ...
  const sendNode = async () => {
    ...
    const payload = {
      name: form.name, mac: form.mac,
      major: +form.major, minor: +form.minor,
      x: +form.x, y: +form.y, z: +form.z,
      left: form.left || '', right: form.right || '',
    }
    setNodes(prev => ({ ...prev, [form.name]: { ...(prev[form.name] || {}), ...payload, placed: true } }))
    setSending(true)
    setToast({ type: '', msg: '' })
    try {
      await axios.post(`${API}/add_node`, payload)
      setToast({ type: 'ok', msg: `✓ ${payload.name} sent` })
      addLog(`Sent node ${payload.name} (${payload.mac}) to device`, 'ok')
    } catch (err) {
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      addLog(`Config updated — Tx power: ${config.meas_power} dBm, PLE: ${config.path_loss_exp}`, 'ok')
    } catch (err) {
              ...
              return (
                <div key="live_pos" style={{
                  ...
                  <div style={{
                    ...
                    POS ({livePos.x.toFixed(2)}, {livePos.y.toFixed(2)})
                  </div>
                </div>
              )
            })()}
            ...
            {Object.values(nodes).map(node => {
              ...
              return (
                <div key={node.name} onClick={e => {
                  ...
                  cursor: 'pointer', opacity: dim ? 0.2 : 1, transition: 'opacity .25s',
                }}>
                  <div style={{
                    width: 38, height: 38, borderRadius: '50%',
                    border: `2px solid ${selected ? '#3b8aff' : active ? '#00e5a0' : '#252b35'}`,
                    background: selected ? 'rgba(59,138,255,.12)' : active ? 'rgba(0,229,160,.08)' : '#1a1e24',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontFamily: 'monospace', fontSize: 9, fontWeight: 600,
                    color: active ? '#00e5a0' : '#c8d4e0',
                    boxShadow: selected ? '0 0 0 2px rgba(59,138,255,.3)' : 'none',
                    transition: 'all .2s',
                  }}>
                    {node.name.replace('NODE_', '')}
                  </div>
                  <span style={{ fontFamily: 'monospace', fontSize: 9, color: '#4a5568', whiteSpace: 'nowrap' }}>
                    {node.name}
                  </span>
                  {active && (
          ...
          <div style={s.tabs}>
            {TABS.map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                ...
                {tab === 'nodes' ? '◈ Nodes' : tab === 'position' ? '⊕ Position' : tab === 'config' ? '⚙ Config' : '▤ Log'}
              </button>
            ))}
          </div>
          ...
          <div style={s.panelBody}>
              ...
              !selectedId ? (
                ...
                <>
                  ...
                  {(live.rssi && live.rssi !== 0) && (
                    <>
                      <hr style={s.divider} />
                      <div style={s.sectionLabel}>Live telemetry</div>
                      <StatRow label="RSSI"     value={`${live.rssi} dBm`}    color="#3b8aff" />
                      <StatRow label="Distance" value={`${live.distance} m`}  color="#00e5a0" />
                      <StatRow label="Status"   value="● Active"              color="#00e5a0" />
                    </>
                  )}
                </>
              )
            ...
            {activeTab === 'config' && (
              <>
                ...
                <StatRow label="Nodes seen" value={Object.keys(nodes).length} color="#c8d4e0" />
            ...
            {activeTab === 'log' && (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <button onClick={clearLog} style={{ ...s.btnSm, fontSize: 9 }}>Clear</button>
                </div>
                <div ref={logRef} style={s.logBox}>
                  {log.length === 0 && (
                    <div style={{ color: '#4a5568', fontFamily: 'monospace', fontSize: 10, padding: 8 }}>No events yet.</div>
                  )}
                  {log.map((entry, i) => (
...
function Pill({ children }) {
  return <div style={{ display:'inline-flex', alignItems:'center', gap:5, background:'#1a1e24', border:'1px solid #252b35', borderRadius:3, padding:'2px 8px', fontFamily:'monospace', fontSize:10, color:'#4a5568' }}>{children}</div>
}
function Dot({ status }) {
...
function Field({ label, children }) {
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:3, marginBottom:9 }}>
      <span style={{ fontFamily:'monospace', fontSize:9, letterSpacing:'.1em', textTransform:'uppercase', color:'#4a5568' }}>{label}</span>
      {children}
    </div>
  )
}
...
function RssiBar({ rssi }) {
  // rssi is negative; -40 = strong, -90 = weak
